...
const STATUS_TRANSITIONS: Record<string, { label: string; next: string; className: string }[]> = {
  WAITING: [
    { label: "✓ เปลี่ยนเป็นออกแล้ว", next: "ISSUED", className: "bg-green-600 hover:bg-green-700 text-white" },
    { label: "✕ ยกเลิก", next: "CANCELLED", className: "bg-red-500 hover:bg-red-600 text-white" },
  ],
  ISSUED: [
    { label: "กลับเป็นรอดำเนินการ", next: "WAITING", className: "bg-blue-600 hover:bg-blue-700 text-white" },
  ],
  CANCELLED: [
    { label: "กู้คืน (รอดำเนินการ)", next: "WAITING", className: "bg-gray-600 hover:bg-gray-700 text-white" },
  ],
};
